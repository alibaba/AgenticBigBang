...
- ``cwd`` is always the case directory so the model can only see files inside
  that case.
- Session persistence is enabled so a cleanly-failed attempt can be resumed
  via ``--resume <session_id>`` (validation-failure retries become cheap
  in-place fixes that reuse the already-read context and its prompt cache).
  The CLI may retain session transcripts; prune them according to local policy.
...
def build_cmd(
    *,
    ...
    cmd: list[str] = [
        ...
        "--output-format",
        "json",
        # Session persistence is ON (no --no-session-persistence): failed
        # attempts that exited cleanly can be resumed via --resume, so the
        # retry reuses the already-read context (and its prompt cache) and
        # only applies a corrective fix instead of re-running the stage.
        # Transcripts land under the CLI session store.
        "--bare",
