...
- findings[] items: {"axis", "severity", "where", "what", "fix"} ONLY — do NOT
  use "issue"/"location" keys.
- fixes_applied[] items are OBJECTS: {"file", "summary"} — never plain strings.
- scores must contain all 11 axis keys as integers 1-5.
"""
...
async def run(
    *,
    ...
    system_prompt_file = compose("4_audit", base_system, case_dir, family_profile)
