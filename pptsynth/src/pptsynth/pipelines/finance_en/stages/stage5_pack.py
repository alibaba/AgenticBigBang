...
Produces ``generation_task/statistics.yaml`` and a per-case ``case_status.json``
summarising the pipeline run. Performs final structural validation.
"""
...
def _coarse_token_estimate(text: str) -> int:
    """Cheap token estimate: ~4 chars per token. Don't pretend to be precise."""
...
def _pack(case_dir: Path) -> _PackResult:
    ...
    if paper_card_path.exists():
        try:
            ...
            paper_card_summary = {
                ...
                "sub_genre": card.get("sub_genre"),
                "reporting_period": card.get("reporting_period"),
    ...
    stats = {
        ...
        "doc_meta": {
            ...
            "sub_genre": case_meta.get("sub_genre"),
            "issuer": case_meta.get("issuer"),
            "reporting_period": case_meta.get("reporting_period"),
            "sector": case_meta.get("sector"),
            "sha256": case_meta.get("sha256"),
        },
        "input_metrics": {
...
def _render_yaml(stats: dict) -> str:
    """Tiny YAML dumper that handles only the shape we use here.

    We intentionally avoid pyyaml as a hard dep. Strings are wrapped in
    double quotes only when they contain risky chars.
    """
...
def _cleanup_case_dir(case_dir: Path) -> list[str]:
    """Remove files we mounted into the case dir as agent scratch.

    Also remove any stray ``_*.py`` scripts the agent may have written
    as scratch (we never authored these; they are leftovers).
    """
    removed: list[str] = []
    for name in _SCAFFOLDING_TO_REMOVE:
        ...
        if p.exists():
            try:
                ...
                pass
    # Stray agent-written python or shell scratch in the case root.
