...
from .family_registry import TITLE_SLIDE_FIELDS
...
def _check_judge(case_dir: Path, issues: list[str]) -> dict | None:
    ...
    if not (9 <= len(c1) <= 32):
        issues.append(f"completeness count out of [9,32]: {len(c1)}")
    if not (8 <= len(c2) <= 32):
        issues.append(f"correctness count out of [8,32]: {len(c2)}")
...
def _cross_consistency(card: dict | None, instructions_text: str | None, judge: dict | None, issues: list[str]) -> None:
    ...
    if title and title not in instructions_text:
        issues.append("note: document title not found verbatim in instructions.md (may be reformatted)")
    # Check that some key_figures are referenced in instructions (topical; not by id).
    ...
    if fig_ids and hits < min(3, len(fig_ids)):
        issues.append(
            f"note: only {hits}/{len(fig_ids)} key_figures referenced by id in instructions.md "
            "(finance decks reference exhibits topically, so this is informational)"
        )
...
def verify_all(out_root: Path) -> list[CaseReport]:
    ...
    economics = out_root / "economics"
    if not economics.exists():
        return []
    return [
        verify_case(d, schemas_dir)
        for d in sorted(economics.iterdir())
        if d.is_dir()
    ]


def main() -> int:
    ap = argparse.ArgumentParser(prog="econ_synth.verify")
