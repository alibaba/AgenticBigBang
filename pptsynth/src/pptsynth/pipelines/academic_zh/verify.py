...
合并自 refineAB2（verify-refine 报告校验、content-driven 条数范围）
和 sinoconf_v3（中文检查、CJK 比例、anti-leakage）。
"""
...
def _cjk_ratio(text: str) -> float:
    ...
    if not non_ws:
        return 0.0
    cjk = sum(1 for c in non_ws if "一" <= c <= "鿿" or "㐀" <= c <= "䶿")
...
def _check_instructions(case_dir: Path, issues: list[str]) -> str | None:
    ...
    if section1:
        ratio = _cjk_ratio(section1)
        if ratio < 0.3:
            issues.append(f"Section 1 CJK char ratio too low ({ratio:.1%}); expected Chinese content")
...
def _check_judge(case_dir: Path, issues: list[str]) -> dict | None:
    ...
    if not (9 <= len(c1) <= 22):
        issues.append(f"completeness count out of [9,22]: {len(c1)}")
    if not (8 <= len(c2) <= 25):
        issues.append(f"correctness count out of [8,25]: {len(c2)}")
    ...
    for i, item in enumerate(c2, 1):
        ...
        elif "如果**否**" not in item and "如果 **否**" not in item and "若**否**" not in item and "If **no**" not in item:
...
def _cross_consistency(card: dict | None, instructions_text: str | None, judge: dict | None, issues: list[str]) -> None:
    ...
    if fig_ids and hits < min(2, len(fig_ids)):
        issues.append(
            f"only {hits}/{len(fig_ids)} key_figures referenced in instructions.md (need >=2)"
        )
...
def verify_all(out_root: Path) -> list[CaseReport]:
    ...
    academia = out_root / "sinoconf_academia"
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="sinoconf_synth.verify")
